 ...
 * Verifies Stripe session is paid, then marks listing paid+live in Supabase.
 */
export default async function handler(req, res) {
  ...
  try {
    ...
    // If stripe_session_id column doesn't exist, retry without it
    if (!patchRes.ok) {
      const text = await patchRes.text()
      if (text.includes('stripe_session_id')) {
        ...
        if (!retry.ok) {
          return res.status(retry.status).json({ error: retryText || 'Failed to activate listing' })
        }
